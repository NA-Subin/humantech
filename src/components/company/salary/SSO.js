...
function SSODetail({ tabState, setTabState, tabId }) {
    ...
    const [sso, setSSO] = useState([{ ID: 0, percent: '', ssomax: '' }]);
    const columns = [
        { label: "เปอร์เซ็นการหัก", key: "percent", type: "text" },
        { label: "ค่าประกันสังคมสูงสุด", key: "ssomax", type: "text" }
    ];
    ...
    const handleSave = () => {
        ...
        const names = sso.map(row => row.deptname?.trim()).filter(Boolean); // ตัดช่องว่างด้วย
        ...
        <Container maxWidth="xl" sx={{ p: 5, width: windowWidth - 290 }}>
            <Box sx={{ flexGrow: 1, p: 5, marginTop: 2 }}>
                <Grid container spacing={2}>
                    <Grid item size={12}>
                        <Typography variant="h5" fontWeight="bold" gutterBottom>ประกันสังคม (Social Security)</Typography>
                    </Grid>
                </Grid>
            </Box>
                        ...
                        <Grid item size={12}>
                            {
                                editSSO ?
                                    <Paper elevation={2} sx={{ borderRadius: 1.5, overflow: "hidden" }}>
                                        {/* <HotTable
                                            data={sso}
                                            afterChange={handleChange(setSSO)}
                                        ...
                                        <Table size="small" sx={{ tableLayout: "fixed", "& .MuiTableCell-root": { padding: "5px" }, width: "100%" }}>
                                            ...
                                            >
                                                <TableRow sx={{ backgroundColor: theme.palette.primary.dark }}>
                                                    ...
                                                    <TablecellHeader>เปอร์เซ็นการหัก</TablecellHeader>
                                                    <TablecellHeader>ค่าประกันสังคมสูงสุด</TablecellHeader>
                                                </TableRow>
                                            </TableHead>
                                            <TableBody>
                                                {
                                                    sso.length === 0 ?
                                                        ...
                                                        sso.map((row, index) => (
                                                            <TableRow>
                                                                ...
                                                                <TableCell sx={{ textAlign: "center" }}>{row.percent}</TableCell>
                                                                <TableCell sx={{ textAlign: "center" }}>{row.ssomax}</TableCell>
                                                            </TableRow>
                                                        ))}
                                            </TableBody>
                                        </Table>
                        ...
                        {/* {
                                    editSSO ?
                                        <Box textAlign="right">
                                            ...
                                            <IconButton variant="contained" color="error" onClick={() => handleRemoveRow("sso")}>
